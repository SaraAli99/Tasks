...
# feach Account ..... 
 ...
 $account_op = mysqli_query($con , $sql);
...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    $account_id = Clean($_POST['account_id']);
    ...
    # Validate account .... 
     if(empty($account_id)){
         $errors['Account'] = "Filed Required";
     } elseif (!filter_var($account_id,FILTER_VALIDATE_INT)) {
        $errors['Account'] = "Invalid Account id";
     }
    ...
    if (count($errors) > 0) {
        ...
        if(move_uploaded_file($imgTemp , $dispath)){
            ...
            $sql = "INSERT INTO `users`(`title`, `content`, `image`, `startDate`, `endDate` , `account_id`) VALUES ('$title','$content','$imgFinalName','$startDate' , '$endDate' , '$account_id')";
...
<body>
    ...
    <div class="container">
        ...
        <form action="<?php echo  htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="exampleInputPassword">Accounts</label>
                <select class="form-control"  name="account_id" >
                 
                <?php 
                   while($acc_data = mysqli_fetch_assoc($account_op)){
                ?>

                <option value="<?php echo $acc_data['id'];?>"><?php echo $acc_data['title'];?></option>
